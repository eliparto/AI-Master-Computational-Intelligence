...
from main import get_actuator_ids, get_brain, objective_function_multi

weights = solutions_mat = np.reshape(np.arange(0, 27, 1), (3,9)) / 30
...
brains = [
    get_brain(gecko(), solutions_mat[i], actuator_ids) for i in range(3)
    ]
# score = simulate_cpg_network(body, brain, render=True, num_steps=5_000)
objective_function_multi(weights, actuator_ids, render=True)